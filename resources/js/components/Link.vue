  ...
  <div class='flex flex-column'>
    ...
    <table>
      <tr>
        ...
        <th>        
          <div class='flex flex-column'>
            <label class='cursor-pointer text-blue-500 original-link-column' @click="this.orderAndChangeInput('original_link')">Original Link</label>
          </div>
        </th>
        <th>        
          <div class='flex flex-column'>
      ...
      <tr v-for="(link, index) in linksData.data" :key="link.id">
        ...
        <td class="original-link-column">{{ link.original_link }}</td>
        <td>{{ link.shorten_link }}{{ link.description }}</td>
        <td>{{ link.link_clicks_count }}</td>
        <td>
          <div class='flex'>
            <input class='border border-gray-800 modify-input' name="description_input" type="text"  v-model="link.description">
